...
6. Copy `.env.example` to `.env` and fill in those values:
   ...
   > Vite only exposes `VITE_`-prefixed env vars to client-side code
   > (`import.meta.env`) by default — anything else silently resolves to
   > `undefined` in the browser bundle. Since these are deliberately named
   > without that prefix (to match what's set in Vercel), `vite.config.ts`
   > explicitly adds `FIREBASE_` to `envPrefix` so they're exposed anyway. If
   > you rename these, update `envPrefix` in `vite.config.ts` to match.
